<template>
  <div>
    <div class="container workarea">
      ...
      <div class="alert alert-success" v-if="success">Проект изменен</div>
      <div class="alert alert-danger" v-if="error">Ошибка</div>
      <form @submit.prevent="submitForm()" v-if="!success">
        <div class="row">
          <div class="col-lg-3 align-self-end mb-3" v-for="(prop, index) in props" :key="index">
            <FormSelect
              :code="prop.code"
              :name="prop.name"
              :list="prop.list"
...
export default Vue.extend({
  ...
  methods: {
    ...
    getProject() {
      this.$store.dispatch('getProjectEdit', this.id).then((projectEdit) => {
        let project = projectEdit

        for(let i = 0; i < this.props.length; i++) {
          let code = this.props[i].code
          let pattern = (this.props[i].type == 'checkbox') ? new RegExp(/true/) : new RegExp(/.+/)
          pattern = (!this.props[i].required) ? new RegExp(/.*/) : pattern

          this.props[i].value = (this.props[i].type == 'checkbox') ? !!parseInt(project[code]) : project[code]
          this.props[i].error = !pattern.test(project[code])
        }
  ...
  mounted() {
    ...
    if(this.$store.state.projectProps.UF_REGION.length > 0 && this.$store.state.projectProps.UF_USER.length > 0 && this.$store.state.projectProps.UF_MANAGER.length > 0){
      ...
      this.$router.push({path: `/project/${this.id}`})
    }
  }
})
</script>